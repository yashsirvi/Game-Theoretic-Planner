...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        # subscriber
        obs_subscriber = self.create_subscription(Odometry, '/ego_obs', self.callback, 10)

        with open('config_example_map.yaml') as file:
            self.conf_dict = yaml.load(file, Loader=yaml.FullLoader)
        self.conf = Namespace(**self.conf_dict)

        self.planner = PurePursuitPlanner(self.conf, (0.17145+0.15875)) #FlippyPlanner(speed=0.2, flip_every=1, steer=10)
    ...
    def callback(self, odom_msg):
        obs_x = odom_msg.pose.pose.position.x
        obs_y = odom_msg.pose.pose.position.y
        obs_theta = odom_msg.pose.pose.position.z # theta is sent through the z coordinate

        speed, steer = self.planner.plan(obs_x, obs_y, obs_theta, PurePursuit.work['tlad'], PurePursuit.work['vgain'])
        new_msg = AckermannDriveStamped()
        new_msg.drive.speed = speed
        new_msg.drive.steering_angle = steer
...
def old_main(args=None):
    ...
    # subscriber
    obs_subscriber = node.create_subscription(Odometry, '/ego_obs', callback, 10)
    ...
    def callback(odom_msg):
        ...
        speed, steer = planner.plan(obs_x, obs_y, obs_theta, work['tlad'], work['vgain'])
        new_msg = AckermannDriveStamped()
        new_msg.drive.speed = speed
        new_msg.drive.steering_angle = steer
        drive_publisher.publish(new_msg)
...
def main(args=None):
    rclpy.init(args=args)
    ackermann_publisher = PurePursuit()
    rclpy.spin(ackermann_publisher)
    ackermann_publisher.destroy_node()
    rclpy.shutdown()


def publish_ackermann_drive(self, steer, speed):
        msg = AckermannDriveStamped()
        msg.header = Header()
        msg.header.stamp = self.get_clock().now().to_msg()
